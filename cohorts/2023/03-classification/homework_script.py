...
required_cols = ['Make', 'Model', 'Year', 'Engine HP', 'Engine Cylinders', 'Transmission Type', 'Vehicle Style', 'highway MPG', 'city mpg','MSRP']
...
print(f'the most frequent observation (mode) for the column transmission_type is {df.transmission_type.mode()[0]}')
...
pairs = [('engine_hp', 'year'), ('engine_hp', 'engine_cylinders'), ('highway_mpg', 'engine_cylinders'), ('highway_mpg', 'city_mpg')]

for i in pairs:
    print(f'the correlation between {i[0]} and {i[1]} is {df[i[0]].corr(df[i[1]])}')
...
df_train_full, df_test = train_test_split(df, test_size=required_split[2]/100, random_state=42)

df_train, df_val = train_test_split(df_train_full, test_size=required_split[1]/(required_split[0]+required_split[1]), random_state=42)
...
print(f'the length of train set is {len(df_train)} , val set is {len(df_val)} and test set is {len(df_test)}') 
...
def prepare_data(df_in : pd.DataFrame, required_features_in: list, dv: DictVectorizer,  data_type : str ):
    df = df_in.copy()
    required_features = required_features_in.copy()
    y  = df[required_features].price
    del df['price']
    required_features.remove('price')
    df_dict = df[required_features].to_dict(orient='records')
    if data_type == 'train':
        return dv.fit_transform(df_dict), y
    else:
        return dv.transform(df_dict), y
...
# correct_predictions = sum(true_label == pred_label for true_label, pred_label in zip(y_val.to_numpy(), df_pred['pred_label'].to_numpy()))
...
for feature in evaluation_features:
    ...
    X_train,y_train = prepare_data(df_train, required_features, dv_selective, 'train')
...
def prepare_regression_data(df_train, df_val,df_test, required_features):
    df_train, df_val, df_test = df_train.copy(), df_val.copy(), df_test.copy()
    required_features = required_features.copy()

    y_train = np.log1p(df_train[required_features].price)
    y_val = np.log1p(df_val[required_features].price)
    y_test = np.log1p(df_test[required_features].price)

    del df_train['price']
    del df_val['price']
    del df_test['price']
    required_features.remove('price')
    numerical_cols_df = [col for col in df_train.columns if df_train[col].dtype != 'object']
    # scaler = StandardScaler()
    ...
    train_dict = df_train[required_features].to_dict(orient='records')
    val_dict = df_val[required_features].to_dict(orient='records')
    test_dict = df_test[required_features].to_dict(orient='records')
    X_train = dv.fit_transform(train_dict)
    X_val = dv.transform(val_dict)
...
for alpha in alphas:
    ...
    X_train, y_train, X_val, y_val, X_test, y_test = prepare_regression_data(df_train,df_val,df_test,categorical + numerical)
